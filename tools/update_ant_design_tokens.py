...
def build_outputs(source_root: Path) -> tuple[bytes, bytes, bytes, bytes]:
    ...
    return json_bytes(lock), catalog_data, schema_data, document_data


def output_paths(repo_root: Path) -> tuple[Path, Path, Path, Path]:
...
def write_outputs(repo_root: Path, outputs: tuple[bytes, bytes, bytes, bytes]) -> None:
...
def verify_repository(repo_root: Path) -> None:
    lock_path, catalog_path, schema_path, document_path = output_paths(repo_root)
...
def self_test(repo_root: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="rynui-token-contract-") as directory:
        ...
        for source, target in zip((lock_path, catalog_path, schema_path, document_path), targets):
            target.parent.mkdir(parents=True, exist_ok=True)
            target.write_bytes(source.read_bytes())
        catalog = json.loads(targets[1].read_text(encoding="utf-8"))
        catalog["entries"][0]["upstream_name"] = "tampered"
        targets[1].write_bytes(json_bytes(catalog))
        try:
