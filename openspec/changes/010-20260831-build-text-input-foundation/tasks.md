...
- [x] 8.4 在一个受支持平台用正式preset运行全部平台通用unit/headless/contract/benchmark、public dependency、lock/license、无网络runtime和Python cache检查，记录实际OS/compiler/preset/result，不要求另一平台重复本组合同
- [x] 8.5 更新`docs/architecture.md`与必要generated Token reference，记录Input/editor/session/utf8proc边界；README只更新用户可见能力与文档入口，AGENTS工作流不得混入README
- [x] 8.6 运行`openspec doctor --json`、`openspec validate --all --strict --no-interactive`与`git diff --check`；以英文 `test: validate input component contracts` 提交并推送平台通用evidence，核对remote SHA
