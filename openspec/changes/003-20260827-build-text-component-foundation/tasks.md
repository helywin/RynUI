...
- [x] 1.4 运行 Prop/Reactive 全部自动测试与 `git diff --check`；以英文 `feat: add reactive prop values` 提交本阶段相关文件，平台 build/CTest 结果分别记录到对应验收清单
...
- [x] 2.5 运行 component/Scope/Node 生命周期自动测试与 `git diff --check`；以英文 `feat: add component composition host` 提交本阶段相关文件，平台 build/CTest 结果分别记录到对应验收清单
...
- [x] 3.5 运行 layout/component/public API 全部自动测试与 `git diff --check`；以英文 `feat: add public layout style` 提交本阶段相关文件，平台 build/CTest 结果分别记录到对应验收清单
...
- [x] 4.5 运行 Font/Text/Glyph/renderer 与多 Text 集成自动测试和 `git diff --check`；以英文 `feat: support multiple text components` 提交本阶段相关文件，平台 build/CTest 结果分别记录到对应验收清单
...
- [x] 5.6 运行 Text component、Prop、Layout、多 Text 与 public API 全部自动测试和 `git diff --check`；以英文 `feat: add public text component` 提交本阶段相关文件，平台 build/CTest 结果分别记录到对应验收清单

## 6. 平台无关示例迁移与 headless 验收
...
- [ ] 7.1 使用 `windows-msvc` preset clean configure，完成 Debug/Release build 与完整 CTest，覆盖第 1–6 节新增的 Prop、component、Layout、多 Text、public API 与 headless contract；保存独立 Windows 结果，不得以 Linux 结果代替
- [ ] 7.2 在 D3D12/DXIL 真实窗口运行示例并正常关闭，保存 display scale、截图、driver、shader format、退出码和全部计数证据，人工核对 14px Latin/CJK、三种 tone、换行、margin 与共享 glyph
- [ ] 7.3 运行 Windows evidence contract、public dependency leak 和 `git diff --check`；以英文 `test: validate Windows text components` 提交本平台验收文件，不把 headless、Linux 或 Windows build 结果描述为 Windows 真实窗口通过

## 8. Linux 验收清单

- [x] 8.1 使用 `linux-gcc` preset clean configure，完成 Debug/Release build 与完整 CTest，核对 Ninja Multi-Config、标准 C++20、BUNDLED FreeType/HarfBuzz/字体和 SPIR-V shader 仍来自锁定来源
- [x] 8.2 使用 `linux-clang` preset 完成 Debug configure/build/CTest，核对 RynUI 自有 Prop/component/Layout/Text/renderer target 使用 `-std=c++20` 而非 GNU extensions；环境缺 Clang 时保持未完成并记录可复现阻塞
- [x] 8.3 在 Linux/GCC/Vulkan/SPIR-V 真实窗口运行公开 Text DSL 示例，触发 content、tone、width、margin 与 resize 更新并正常退出；保存 display scale、截图、driver、退出码和计数证据，核对三种 tone、局部更新、共享 atlas、稳定 draw order 与 idle
- [x] 8.4 运行 Linux `BUNDLED|SYSTEM` dependency contract、public-header isolation/leak、shader/lock/license 与未跟踪字体检查，证明 003 不新增第三方 library、运行时字体扫描、submodule 或 system-first fallback
- [x] 8.5 运行 Linux evidence contract、完整 CTest 与 `git diff --check`；以英文 `test: validate Linux text components` 提交本平台验收文件，不把 Linux 结果描述为 Windows 通过

## 9. Change 收口

- [ ] 9.1 在准备 archive 时运行 `openspec doctor --json`、`openspec validate --all --strict --no-interactive` 与 `git diff --check`，确认所有独立验收条目已完成、worktree 只包含本 change 相关文件且 README、AGENTS、architecture 与 OpenSpec 职责未混写；本项只校验 change 完整性，不替代任何平台验收
