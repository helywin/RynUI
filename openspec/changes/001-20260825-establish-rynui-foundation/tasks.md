...
- [x] 5.3 实现 frame request 与按需提交状态机，通过可控 clock/event 测试证明更新会请求帧、稳定后不按刷新率持续 submit、后续输入能重新唤醒
- [x] 5.4 创建彩色 Quad 真实窗口示例，输出 Component、Signal、Observer、Measure、Layout、Primitive、GPU upload、submit 和 idle wake 计数
- [x] 5.5 在 Windows 真实窗口触发 Material、Transform 和 size 更新，核对视觉结果与计数关系，执行全部测试和 `git diff --check` 后使用英文 commit message 提交本阶段

## 6. Linux 验收与 change 收口

- [x] 6.1 在 Linux/Vulkan 环境完成 configure、build、CTest 和真实窗口运行，记录 GPU driver、shader format、退出码与计数摘要
- [x] 6.2 在 Windows/D3D12 环境重新运行 clean configure、build、CTest 和真实窗口验收，确认 `BUNDLED|SYSTEM` 选择与文档一致
- [x] 6.3 运行 `openspec doctor --json`、`openspec validate --all --strict --no-interactive` 和 `git diff --check`，更新任务与验收证据但不得提前勾选未通过的平台结果
- [x] 6.4 确认 worktree 只包含本 change 相关文件，使用英文 commit message 提交最终验收阶段，并记录最终 commit SHA
