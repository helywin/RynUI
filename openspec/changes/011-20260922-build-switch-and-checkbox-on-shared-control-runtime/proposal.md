...
RynUI 已有 Button、Input 以及响应式属性、焦点、指针、动画和 retained scene 基础，但部分窗口级服务仍由 Button 宿主持有，按压与激活规则也未形成可组合的内部机制。继续逐个复制组件宿主会使当前 73 项目录的后续实现越来越慢且难以保持一致；现在用两个选中型控件验证共享边界。
